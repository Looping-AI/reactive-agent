...
Every accepted Task runs a **round loop** in a durable Workflow: pre-work → a **round** (one main-agent inference that either answers the user or delegates 1-8 durable Subtasks) → **execute** the dependency-ready ones concurrently in isolated managed subagents → back to the next round, which sees the results and decides again → deliver. The agent is never forced either way: plain text is a terminal answer, `delegate` is a control tool, and only the last round of the budget is denied the choice. See "The round loop" in [ARCHITECTURE.md](ARCHITECTURE.md). The main agent's work tools are `browser_*`, gated `recall`, and the ARC scorecard lifecycle (`arc_list_games` / `arc_list_scorecards` / `arc_open_scorecard` / `arc_close_scorecard`), layered over the Session's own `set_context`; subagents get the `browser`/`workspace`/domain families from their Recipe — the `arc-game` family plays a game and can neither open nor close a card. Subtask **types** are a closed set that may require **params** ([src/agent/subtasks/subtask-types.ts](src/agent/subtasks/subtask-types.ts)): the type owns the contract (`arc-game` needs `card_id` + `game_id`), the Recipe is only the configuration it runs under, and a type's tool family is withheld when its params are absent. Params are fingerprinted as part of an execution's identity; the session state the parent resolves _from_ them (the ARC cookie jar, which the API pins a card to) travels beside the request and is not. Per-call **authorization** policy for domain tools is a later phase. The enduring value is the zero-trust _contract_, which is independent of the agent's behavior.
...
- **Gotcha — workflow "errors" are usually noise.** A telemetry event with `outcome: exception` on a Workflow is the runtime _suspending between steps by throwing_, not a failure. Confirm with `wf <name> <instanceId>` and read the instance's `status` / `error` before trusting it.
- **`RecipeSubagent.executeChunk` `outcome: exception` is not a failed Subtask.** Historically _every_ successful chunk emitted one: the parent deleted the child facet (`deleteSubAgent` → `ctx.facets.delete`, which aborts the facet) in the same tick the chunk's RPC returned, and that abort was charged to the already-successful invocation (so its `wallTimeMs` equalled the model call). That eager delete is gone — deletion is deferred to a single post-delivery `sweepTaskChildren` step ([handle-task.ts](src/workflows/handle-task.ts) `deliver` → the DO's `sweepTaskChildren`), which runs when no chunk RPC is open — so a successful chunk now records no exception. A genuine Subtask failure instead surfaces as a labelled parent log: `[handle-task] subtask execution exhausted retries` (retries exhausted) or `[reactive-agent] stale subagent state, recreating` (a real fingerprint mismatch — benign on the first, a lifecycle bug on a second). Pivot to `wf <instanceId>` for the instance's true status either way.
...
- `ARC_API_KEY` — ARC-AGI-3 API key (from the ARC-AGI-3 web console), used by the main agent's scorecard tools and the `arc-game` recipe's tool family. Same `.dev.vars` / `wrangler secret put` flow.
