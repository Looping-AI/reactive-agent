...
The caller-owned `ReactiveAgent` remains the durable main agent and owner of the continuous Session. Every incoming Task is decomposed into one to eight durable Subtasks. Dependency-ready Subtasks run concurrently through trusted Recipes and isolated Agents SDK subagents. The main agent composes multiple outcomes, while a single successful Subtask bypasses composition. Phase 4 retains the signed asynchronous A2A delivery contract.
...
- A Subtask has three distinct input categories:
  ...
  - `referenceParts`: exact, immutable conversation parts selected from a numbered reference catalog.
  - `dependencyResults`: generated output from prerequisite Subtasks.
- Generated instructions and dependency results are never represented as original user or assistant evidence.
- The decomposition model selects reference order IDs only. Application code resolves those IDs to exact source parts; the model never rewrites, summarizes, fabricates, or relabels reference content.
- Filtered reference order remains visible. A Subtask may receive parts 5, 6, 10, 11, 12, 20, 22, 26, and 27; gaps are not renumbered away.
- Compaction summaries, recall results, system/context blocks, and tool output are not eligible reference parts.
- Recipes are trusted deploy-time code. Model output can select a known Recipe type but cannot select model IDs, tools, bindings, or capabilities.
...
- Dependency edges form a validated DAG through `dependsOn: string[]`.
...
Add `src/subtasks/types.ts` with plain, serializable contracts suitable for native Durable Object RPC.
...
- `SubtaskStatus`:
  ...
  - `skipped`
  - `canceled`
- `SubtaskPart`: a metadata-free union corresponding to A2A text, file, and data parts.
- `ReferencePart`:
  - `order`: position in the decomposition snapshot.
  - `messageId`: durable source message identity.
  - `partIndex`: source index within the message.
  - `role`: original `user` or `assistant` role.
  - Available author/channel provenance.
  - Exact metadata-free source part.
- `Subtask`:
  - `id`
  - Parent `taskId`
  - `ordinal`
...
### A3. Build the immutable reference catalog

Build a deterministic flattened catalog during Phase 1 from:
...
- Assign monotonically increasing snapshot order values.
- Retain message ID, part index, role, and available author/channel provenance.
- Preserve exact text, file, and data payloads.
- Exclude Session compaction summaries, recall-search results, system prompts, context blocks, and tool output.

The main agent may use summaries and recall while reasoning, but it may delegate only exact catalog references.

### A4. Add durable Subtask storage

Extend `src/db/schema.ts` with a `subtasks` table keyed by Subtask ID and indexed by parent Task/order and status.
...
- Non-session prompt.
- Reference parts and provenance JSON.
- Dependencies JSON.
...
Wire it as `db.subtasks` in `src/db/db.ts`. Extend the existing 30-day cleanup so expired parent Task rows and their Subtasks are removed together.
...
- Exact reference parts can be selected by order and retain durable provenance.
...
## Phase B: Trusted Recipes and Managed Subagents

### B1. Add the static Recipe registry

Create `src/recipes/` with a typed, deploy-time registry.

A Recipe contains trusted configuration:

- Key and version.
- Primary and fallback model factories or identifiers.
- Dedicated soul.
- Allowed tool factory keys.
- Required symbolic binding keys.
- Accepted input and output part kinds.
- Tool-loop limits.

Bindings are symbolic allowlisted names resolved inside the Worker. No model or inbound payload can supply arbitrary bindings, tools, models, or secrets.

Initially implement only `general`:

- Current primary/fallback Workers AI model pair.
- A dedicated stateless execution soul.
- Browser Rendering tools.
- No Session `set_context` tool.
- No episodic recall.
- No parent memory access.

Derive the decomposition Recipe-type schema from registry keys. Keep a mandatory `general` fallback in the registry for future compatibility.
...
1. Resolve the trusted Recipe locally by type.
2. Build a fresh invocation with clearly separated sections:
   - Trusted Recipe soul.
   - Main-agent instruction prompt.
   - Original conversation references with `[part N]` labels, role, and provenance.
...
Reference rendering must not summarize, rewrite, interpolate, or present dependency output as user-authored evidence.
...
- Accept model, binding, or tool configuration in its request.
...
- Primary-model failure falls back to the configured fallback model.
...
- Recipe capabilities are fully determined by trusted code.
- The general Recipe runs without Session or parent-memory access.
...
2. Read the current system prompt, history, and numbered reference catalog.
...
- Ascending unique `referenceOrders: number[]`.
...
- Validate every selected reference order against the catalog.
- Resolve order IDs to exact `ReferencePart` copies.
- Preserve visible order gaps.
- Normalize local keys to stable parent-scoped Subtask IDs.
...
- The model selects references by order only; exact source parts remain byte-for-byte preserved.
...
- `test/subtasks/scheduler.spec.ts`
  ...
  - Cancellation.
- `test/recipes/recipes.spec.ts`
  - Registry uniqueness.
  - Mandatory general Recipe.
  - Trusted tool/binding resolution.
  - No untrusted capability selection.
- `test/subagent/subagent.spec.ts`
  - Fresh stateless prompts.
  - Exact reference rendering with visible order gaps.
...
- `test/agent/`
  ...
  - Index-only reference selection.
  - Ascending/unique/unknown order validation.
  - Exact source-part preservation.
...
- Reference provenance and visible order gaps.
...
2. Recipe registry and stateless managed Subagent.
...
- Dynamic or user-editable Recipes.
